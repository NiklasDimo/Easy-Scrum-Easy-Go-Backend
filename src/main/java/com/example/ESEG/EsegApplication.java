...
public class EsegApplication {
	...
	CommandLineRunner commandLineRunner(ProductRepository repository, ImpressumRepository impressumRepository, UserRepository userRepository) {
		return args -> {
			...
			userRepository.save(new User( "admin", "password", "Admin", "0"));
			userRepository.save(new User( "user1", "1", "Standard Customer", "0"));
			userRepository.save(new User( "user2", "1", "Premium Customer", "10"));
		};
	}
}
